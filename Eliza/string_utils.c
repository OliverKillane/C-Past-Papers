...
{
  char* str = calloc(1,1);
  NULLCHECK(str, "empty_string: allocation failure")
  return str;
}
...
{
  char* copy = strcpy(malloc(strlen(str) + 1), str);
  NULLCHECK(copy, "clone: allocation failure")
  return copy;
}
...
char *push_string(char *current, const char *append)
{
  ...
  NULLCHECK(current, "push_string: reallocation failed")
