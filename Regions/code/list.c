...
void list_insert(list_iter iter, region_t *region)
{
  ...
  list_elem_t *new_item = alloc_list_elem();
  new_item->region = region;

  new_item->next = iter;
  new_item->prev = iter->prev;

  /* link with others */
  iter->prev->next = new_item;
  iter->prev = new_item;
}
...
{
  list_elem_t* item = list->header->next;
  for (; item->next; item = item->next) {
    if (region_compare(region, item->region)) {
      break;
    }
  }
  
  list_insert(item, region);
}
...
{
  for (list_iter current = list_begin(list), next; current->next; current = next) {
    next = list_iter_next(current);
    region_destroy(current->region);
    free(current);
  }
